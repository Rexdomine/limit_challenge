...
source_render_env() {
  ...
  if [[ -f "$STATE_RENDER_ENV" ]]; then
    # Local Render credentials are intentionally stored outside tracked repo files.
...
echo "Using preview API base URL: $NEXT_PUBLIC_API_BASE_URL"
...
if [[ -z "$DEPLOY_URL" ]]; then
  echo "FAIL: could not determine preview deployment URL from Vercel output." >&2
  exit 1
fi
